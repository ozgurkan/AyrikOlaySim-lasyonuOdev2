...
% caller's status.
impatientCallers = 0;   %sabýrsýz arayan sayisi 
    ...
    switch(Caller(k))
        case 'P'    % the action is a call attempt
            if serveLineNum <2      % there are still line available
                ...
                EventList(k) = genexprv(mu, 1) + currentTime;
            else
                Caller(k) = 'I';    %blocked, become impatient
                impatientCallers = impatientCallers + 1;
                a(i)=currentTime;
                b(i)=impatientCallers;
                % the next event happens when the impatient caller calls
                % again
                EventList(k) = genexprv(2*lambda, 1) + currentTime;
            end
        ...
        case 'C'	% the action is finishing phone call
            ...
            % lambda
            EventList(k) = genexprv(lambda, 1) + currentTime; 
    end
...
figure;
stem(a,b) %impatientCallers zamana göre grafiði stemler ile çizdirildi
legend('Number of callers impatient');
xlabel('Time (t)'); 
ylabel('Number of callers impatient');
set(gcf,'Position', get(gcf,'Position') + [0,0,250,0]); 
pos2 = get(gcf,'Position');  
set(gcf,'Position', pos2 + [pos1(3)/3,0,0,0])

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This function generates samples of exponential distribution with rate 
% lambda, whose PDF is given in the question
function [samples] = genexprv(lambda, noSamples)
   %
   % Write your own code here
   %
    [samples]=-log(rand)/lambda; 
end
